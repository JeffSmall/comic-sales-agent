...
_system_prompt = _schema_mgr.generate_system_prompt(
    ...
    workflow_description=(
        ...
        "- WATCHLIST view (request \"show me my watchlist\", or a back tap): a single Column whose "
        "children are one TAPPABLE Button per comic. Each Button: \"variant\":\"borderless\", "
        "action {\"event\":{\"name\":\"view_book:<book_id>\"}} (REAL book_id from get_watchlist, "
        "e.g. \"view_book:amazing-fantasy-15\"), and \"child\" is ONE Text reading "
        "\"<title> <issue> — <grade/grader, or Raw> — last $<price>\". Keep the watchlist COMPACT: "
        "exactly one Button + one Text per comic, and use SHORT component ids (e.g. "
