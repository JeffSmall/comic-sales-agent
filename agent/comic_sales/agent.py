...
_system_prompt = _schema_mgr.generate_system_prompt(
    ...
    workflow_description=(
        ...
        "existing book_id (from get_watchlist); to add, leave book_id empty. WATCHLIST LIMIT: "
        "the user may track at most 12 comics (D11). Before adding a NEW comic (empty book_id), "
        "call get_watchlist first; if it already returns 12 comics, do NOT add — instead render "
        "ONE Card whose Text explains the watchlist is full (max 12) and that they should remove "
        "a comic before adding another. Editing an existing comic is always allowed.\n"
